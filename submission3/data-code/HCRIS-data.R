...
final.hcris.v1996=read_tsv('data/output/HCRIS_Data_v1996.txt') %>%
  select(-c(uncomp_care, pps_ip_charges, pps_op_charges, pps_mcare_cost, pps_pgm_cost))
final.hcris.v2010=read_tsv('data/output/HCRIS_Data_v2010.txt') %>%
  select(-c(tot_uncomp_care_charges, tot_uncomp_care_partial_pmts, bad_debt, depr_HIT))
...
final.hcris=rbind(final.hcris.v1996,final.hcris.v2010) %>%
  ...
  mutate(tot_charges=as.numeric(tot_charges)) %>%

q5 <- HCRIS_data %>%
  ...
  summarize(
    total_hospitals = n(),
    hrrp_penalized = sum(hrrp_payment > 0, na.rm = TRUE),
    hvbp_penalized = sum(hvbp_payment < 0, na.rm = TRUE),
    hrrp_share = hrrp_penalized / total_hospitals,
    hvbp_share = hvbp_penalized / total_hospitals
  ) %>%
  pivot_longer(cols = c(hrrp_share, hvbp_share),
...
q5.plot <- ggplot(q5, aes(x = year, y = Share, color = Program, group = Program)) +
  ...
  scale_color_manual(values = c("hrrp_share" = "orange", "hvbp_share" = "darkblue"),
                     labels = c("HRRP (Readmissions)", "HVBP (Value-Based)")) +
  labs(
